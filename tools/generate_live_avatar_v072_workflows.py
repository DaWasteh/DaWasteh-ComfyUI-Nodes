#!/usr/bin/env python3
"""Clean-room deterministic generator for Live Avatar 06–11; never queries live ComfyUI."""
...
def build11():return (ASSETS/'workflow-11.template.json').read_bytes()
def generate(destination=DIR):
 destination=Path(destination);destination.mkdir(parents=True,exist_ok=True)
 with tempfile.TemporaryDirectory(prefix='live-avatar-v072-') as d:
  stage=Path(d)
  for name,data in zip(NAMES,(build06(),build07(),build08(),build09(),build10(),build11())):(stage/name).write_bytes(data)
